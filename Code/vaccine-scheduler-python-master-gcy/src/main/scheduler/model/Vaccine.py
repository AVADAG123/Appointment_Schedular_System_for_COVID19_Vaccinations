...
class Vaccine:
    def __init__(self, vaccine_name, available_doses):  # 初始化要求必须传入这俩参数，缺一不可（没设置default）
    ...
    def get(self):
        ...
        cursor = conn.cursor(as_dict=True)  # ed上有人问了，加上会少bug
    ...
    def increase_available_doses(self, num):
        ...
        # 这里很显然规定了类型是%d，这是因为VACCINES table里Doses类型是int
    ...
    def decrease_available_doses(self, num):
        ...
        try:
            ...
            print("Error occurred when updating vaccine availability")  # 所有print的信息都会输出到console
            cm.close_connection()
        cm.close_connection()
    
    # 通过自定义__str__，是的print该类的实例时可以以__str__的形式输出
    def __str__(self):
